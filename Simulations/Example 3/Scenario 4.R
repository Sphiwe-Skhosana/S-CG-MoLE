...
simulate_data <- function(n,x, beta1, beta2,sigma1,sigma2) {
  ...
  x1=x[,1];x2=x[,2]
  pi_u <- exp(-0.1*x1+0.1*x2)/(1+exp(-0.1*x1+0.1*x2))
  ...
  for (i in setdiff(1:n,idx)) {
    if (z[i] == 1) {
      y[i] <- beta1[1] + beta1[2] * x1[i] + beta1[3]*x2[i] + rnorm(1,0,sigma1)
    } else {
      y[i] <- beta2[1] + beta2[2] * x1[i] + beta2[3]*x2[i]+ rnorm(1,0,sigma2)
    }
  }
...
for(n in c(200,500,1e3)){
  ...
  pi_u <- exp(-0.1*x[,1]+0.1*x[,2])/(1+exp(-0.1*x[,1]+0.1*x[,2]))
  count=0
