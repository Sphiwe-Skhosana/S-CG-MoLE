# Function to simulate data from mixture of regressions with contaminated Gaussian distributed errors
simulate_data <- function(n,x, beta1, beta2, sigma1,sigma2, eta, alpha) {
  ...
  for(i in 1:n){
    if(z[i]==1){
      y[i]=rCN(1,beta1[1]+beta1[2]*x[i],cbind(sigma1),alpha,eta)}else{
        y[i]=rCN(1,beta2[1]+beta2[2]*x[i],cbind(sigma2),alpha,eta)}
  }
...
for(n in c(200,500,1e3)){
  ...
  while(count<num_samples) {
    ...
    data <- simulate_data(n,x, beta1, beta2,sigma1,sigma2,eta,alpha)
    init.model=list(beta0=cbind(beta1,beta2),pi0=cbind(data$true_pi,1-data$true_pi))
    try({model1 <- GMLRs.fit(x=data$x, y=data$y,init.beta = init.model$beta0,init.sigma=init.model$sigma0,init.pi=colMeans(init.model$pi0))},silent = T)
    try({model2 <- CGMLRs.fit(x=data$x, y=data$y,init.beta = init.model$beta0,init.sigma=init.model$sigma0,init.pi=colMeans(init.model$pi0))},silent = T)
    try({model3 <- GMoLE.fit(x=data$x, y=data$y,k=k,init.beta = init.model$beta0,init.sigma=init.model$sigma0,init.pi=init.model$pi0)},silent = T)
    try({model4 <- CGMoLE.fit(x=data$x, y=data$y,init.beta = init.model$beta0,init.sigma=init.model$sigma0,init.pi=init.model$pi0)},silent = T)
    try({model5 <- S_GMoLE.fit(x=data$x,y=data$y, max_iter = 1e3, K=k,init.beta = init.model$beta0,init.sigma=init.model$sigma0,init.pi=init.model$pi0)},silent = T)
    try({model6 <- S_CG_MoLE.fit(x=data$x, y=data$y,max_iter = 1e3,init.beta = init.model$beta0,init.sigma=init.model$sigma0,init.pi=init.model$pi0)},silent = F)
    ##Store results
