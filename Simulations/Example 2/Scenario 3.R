...
simulate_data <- function(n,x, u, beta1, beta2, df = 4) {
  ...
  return(list(x = x, u=u, y = y, true_pi = pi_u, true_z = z))
}
...
for(n in c(200,500,1e3)){
  ...
  while(count<num_samples) {
    ...
    data <- simulate_data(n,x,u, beta1, beta2,df=4)
    ...
    try({model3 <- GMoLE.fit(x=data$x,u=u, y=data$y,k=k,init.beta = init.model$beta0,init.pi=init.model$pi0)},silent = T)
    try({model4 <- CGMoLE.fit(x=data$x,u=u, y=data$y,init.beta = init.model$beta0,init.pi=init.model$pi0)},silent = T)
    try({model5 <- S_GMoLE.fit(x=data$x,u=u,y=data$y, max_iter = 1e3, K=k,init.beta = init.model$beta0,init.pi=init.model$pi0)},silent = T)
    try({model6 <- S_CG_MoLE.fit(x=data$x,u=u, y=data$y,max_iter = 1e3,init.beta = init.model$beta0,init.pi=init.model$pi0)},silent = F)
    ##Store results
